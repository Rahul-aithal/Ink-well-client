...
function NavBar({ className }) {
  ...
  return (
    <div className="max-w-screen md:max-w-[100dvw] p-4 md:px-5 sticky top-2 inset-x-0 mx-auto z-50 bg-white bg-opacity-20 dark:bg-black dark:bg-opacity-20 dark:hover:bg-opacity-80 dark:border-b dark:border-gray-700 shadow-md md:flex md:justify-between md:items-center rounded-full">
      ...
      <ul
        ref={ulRef}
        className="md:flex md:items-center md:static absolute z-[-1] md:z-auto bg-gray-100 dark:bg-gray-950 md:bg-opacity-0 md:dark:bg-opacity-0 left-0 w-full md:w-auto md:py-0 py-4 pl-0 md:pl-7 opacity-0 md:opacity-100 top-[-400px] md:top-auto transition-all ease-in-out duration-500"
      >
        <li className="my-4 text-left ms-5 md:my-auto">
          <NavLink
            to={isSignined ? "/dashboard" : "/"}
            className={({ isActive }) =>
              `${
        ...
        {isSignined && (
          <li className="my-4 text-left ms-5 md:my-auto">
            <Button
              onClick={handleLogut}
              className="rounded-xl h-10 max-w-24 p-2"
            >
              Log Out
            </Button>
          </li>
